...
def get_optimizer(cfg, q_network):
    ...
    optimizer = None
    if cfg.enable_muon:
        # optimizer = xAdam(q_network.parameters(), lr=cfg.learning_rate, eps=cfg.adam_eps)
    ...
    if cfg.enable_adams:
        optimizer = Adams_ZeRO(q_network.parameters(), lr=cfg.learning_rate,  weight_decay=cfg.adams_weight_decay, scalar_vector_weight_decay=0.1, betas=(0.9, 0.95))
    if cfg.enable_adam:
        optimizer = optim.Adam(q_network.parameters(), lr=cfg.learning_rate, betas=(0.9, 0.95))
    return optimizer