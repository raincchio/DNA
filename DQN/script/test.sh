...
#      echo ${wd}
      ...
      do
        ...
        i+=1
        misc="--enable_adam --enable_redo --wob --adams_weight_decay ${wd} --redo_check_interval ${itv}"
        cmd="nohup ${python} -um dqn --seed=${seed} --env_id=${env} ${misc} --exp_name  test_eps_dqn_redo_wob  >>/dev/null 2>&1 &"
